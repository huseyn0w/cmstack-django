...
| F12 | **Public REST API** + **MCP server** | ◐ IN PROGRESS | L | ☑ **F12a read API** (DRF; new `apps.api`: ReadOnly viewsets for posts/pages/services/categories/tags at `/api/v1/`, parler-aware serializers, `?lang=` override, published-only, list/detail split; `/health/` + `/health/ready/` DB probe; viewsets delegate to `api.services`→content repos; 12 tests). ☐ F12b gated write API. ☐ F12c MCP server (OAuth-floor + ts tool list). |
